...
/*	float Ka, Kd, Ks;
	...
	dx = abs(sin((Time+dx)* M_PI) );
	dy = abs(cos((Time + dx) * M_PI) )*0.5;
	dz = tan(3 * (Time + M_PI / 2 * M_PI));
	...
	if (AniVer) {
		Pattern->SetUniformVariable("Dx", dx);
		Pattern->SetUniformVariable("Dy", dy);
		Pattern->SetUniformVariable("Dz", dz);
	}
